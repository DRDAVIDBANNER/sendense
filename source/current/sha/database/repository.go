// Package database provides repository layer for SHA database operations
...
// GetByVMContextID retrieves ALL VM disks for a given VM context ID
// 🆕 NEW (2025-10-07): Supports multi-disk VM backup operations
func (r *VMDiskRepository) GetByVMContextID(vmContextID string) ([]VMDisk, error) {
	...
	var disks []VMDisk
	if err := r.db.Where("vm_context_id = ?", vmContextID).Order("disk_id ASC").Find(&disks).Error; err != nil {
		log.WithError(err).WithField("vm_context_id", vmContextID).Error("Failed to retrieve VM disks by context ID")
	...
	log.WithFields(log.Fields{
		...
		"disk_count":    len(disks),
	}).Debug("Retrieved all VM disks for context")

	return disks, nil
}

// GetByVMContextAndDiskID retrieves a VM disk by VM context ID and disk ID
// 🆕 NEW (2025-10-06): Supports backup operations using discovery-populated vm_disks
func (r *VMDiskRepository) GetByVMContextAndDiskID(vmContextID string, diskID int) (*VMDisk, error) {
	if r.db == nil {
		return nil, fmt.Errorf("database not available")
	}

	var disk VMDisk
	// Query by vm_context_id and disk_id (disk_id is stored as string in db like "disk-2000")
	diskIDStr := fmt.Sprintf("disk-%d", diskID+2000) // Convert disk_id 0 -> "disk-2000", 1 -> "disk-2001"
	
	if err := r.db.Where("vm_context_id = ? AND disk_id = ?", vmContextID, diskIDStr).First(&disk).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, fmt.Errorf("disk %d not found for VM context %s", diskID, vmContextID)
		}
		log.WithError(err).WithFields(log.Fields{
			"vm_context_id": vmContextID,
			"disk_id":       diskID,
		}).Error("Failed to retrieve VM disk by context and disk ID")
	...
	log.WithFields(log.Fields{
		"vm_context_id":  vmContextID,
		"disk_id":        diskID,
		"disk_id_str":    diskIDStr,
		"size_gb":        disk.SizeGB,
		"capacity_bytes": disk.CapacityBytes,
	}).Debug("Retrieved VM disk by context and disk ID")
...
// Used by SNAProgressPoller to construct NBD export names
...
// Schema verified from: source/current/sha/database/models.go ReplicationJob struct

...
func NewEnhancedFailoverHandler(db database.Connection) *FailoverHandler {
	...
	if snaClient == nil {
		log.Error("Failed to initialize SNA client for power management - using null client")
		// Continue with null client - power management will be disabled
		snaClient = failover.NewNullVMAClient()
	}
	enhancedCleanupService := failover.NewEnhancedCleanupService(db, jobTracker, snaClient)
	...
	// SNA client already initialized above for cleanup service

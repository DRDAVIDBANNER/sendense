// Package services provides VMA VM information service for network mapping
package services
...
// VMAVMInfoService provides VM information from VMA via control API
type VMAVMInfoService struct {
	vmaClient VMAControlClient
}

// VMAControlClient interface for VMA control API communication
type VMAControlClient interface {
	GetVMInfo(vmID string) (*models.VMInfo, error)
	DiscoverVMs(filter string) ([]models.VMInfo, error)
}

// NewVMAVMInfoService creates a new VMA VM info service
func NewVMAVMInfoService(vmaClient VMAControlClient) *VMAVMInfoService {
	return &VMAVMInfoService{
		vmaClient: vmaClient,
	}
}

// GetVMNetworkConfiguration retrieves VM network configuration from VMA
func (vvis *VMAVMInfoService) GetVMNetworkConfiguration(vmID string) ([]SourceNetworkInfo, error) {
	log.WithField("vm_id", vmID).Debug("Getting VM network configuration from VMA")

	// Get VM info from VMA
	vmInfo, err := vvis.vmaClient.GetVMInfo(vmID)
	if err != nil {
		return nil, fmt.Errorf("failed to get VM info from VMA: %w", err)
	}
	...
	}).Debug("✅ Retrieved VM network configuration from VMA")

	return sourceNetworks, nil
}

// ValidateVMExists validates that a VM exists in VMA
func (vvis *VMAVMInfoService) ValidateVMExists(vmID string) error {
	log.WithField("vm_id", vmID).Debug("Validating VM exists in VMA")

	// Try to get VM info
	vmInfo, err := vvis.vmaClient.GetVMInfo(vmID)
	if err != nil {
		return fmt.Errorf("VM %s not found in VMA: %w", vmID, err)
	}

	if vmInfo == nil {
		return fmt.Errorf("VM %s not found in VMA", vmID)
	}

	// Additional validation checks
	if vmInfo.ID == "" {
		return fmt.Errorf("VM %s has empty ID in VMA", vmID)
	}

	if vmInfo.Name == "" {
		return fmt.Errorf("VM %s has empty name in VMA", vmID)
	}
...
func (vvis *VMAVMInfoService) GetCompleteVMInfo(vmID string) (*models.VMInfo, error) {
	log.WithField("vm_id", vmID).Info("Getting complete VM information from VMA")
...
func (vvis *VMAVMInfoService) GetVMNetworkSummary(vmID string) (map[string]interface{}, error) {
...
func (vvis *VMAVMInfoService) DiscoverVMsWithNetworkInfo(filter string) ([]models.VMInfo, error) {
	log.WithField("filter", filter).Info("Discovering VMs with network information from VMA")

	vms, err := vvis.vmaClient.DiscoverVMs(filter)
	if err != nil {
		return nil, fmt.Errorf("failed to discover VMs from VMA: %w", err)
	}
...
func (vvis *VMAVMInfoService) ValidateNetworkConnectivity(vmID string) (map[string]interface{}, error) {
...
func (vvis *VMAVMInfoService) GetVMNetworkCapabilities(vmID string) (map[string]interface{}, error) {

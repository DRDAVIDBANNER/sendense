...
import (
	...
	"github.com/vexxhost/migratekit-oma/internal/validation"
...
// DetectOMAVMResponse represents OMA VM detection response
type DetectOMAVMResponse struct {
	Success bool                  `json:"success"`
	OMAInfo *validation.OMAVMInfo `json:"oma_info,omitempty"`
	Message string                `json:"message"`
	Error   string                `json:"error,omitempty"`
}
...
type ValidateSettingsRequest struct {
	...
	APIKey            string `json:"api_key"`
	SecretKey         string `json:"secret_key"`
	OMAVMID           string `json:"oma_vm_id,omitempty"`
...
// DetectOMAVM attempts to auto-detect the OMA VM by MAC address
...
func (h *CloudStackSettingsHandler) DetectOMAVM(w http.ResponseWriter, r *http.Request) {
	...
	log.Info("🔍 Attempting to auto-detect OMA VM by MAC address")

	// Create temporary client
	client := ossea.NewClient(req.APIURL, req.APIKey, req.SecretKey, "", "")
	ctx := context.Background()

	// Create validator and detect OMA VM
	validator := validation.NewCloudStackValidator(client)
	omaInfo, err := validator.DetectOMAVMID(ctx)
	if err != nil {
		log.WithError(err).Warn("OMA VM auto-detection failed")
		respondJSON(w, http.StatusOK, DetectOMAVMResponse{
			Success: false,
			Message: "Could not auto-detect OMA VM. Please enter the VM ID manually.",
			Error:   sanitizeError(err),
		})
		return
	}
	...
	}).Info("✅ OMA VM detected successfully")
...
func (h *CloudStackSettingsHandler) ValidateSettings(w http.ResponseWriter, r *http.Request) {
	...
	validator := validation.NewCloudStackValidator(client)
	result := validator.ValidateAll(ctx, req.OMAVMID, req.ServiceOfferingID, req.NetworkID)
...
func sanitizeError(err error) string {
	...
	if contains(errStr, "account") && contains(errStr, "does not match") {
		return "The API key belongs to a different CloudStack account than the OMA VM. Please use credentials from the same account."
	}
...
// DiscoverAllResources combines connection test, OMA VM detection, and resource discovery
...
func (h *CloudStackSettingsHandler) DiscoverAllResources(w http.ResponseWriter, r *http.Request) {
	...
	// Step 2: Detect OMA VM by MAC address
	validator := validation.NewCloudStackValidator(client)
	omaInfo, err := validator.DetectOMAVMID(ctx)
	if err != nil {
		respondJSON(w, http.StatusInternalServerError, map[string]string{
			"error": fmt.Sprintf("Failed to detect OMA VM: %v", sanitizeError(err))})
		return
	}
	...
	response := map[string]interface{}{
		"oma_vm_id":         omaInfo.VMID,
		"oma_vm_name":       omaInfo.VMName,
		"zones":             zonesList,

-- =============================================================================
-- SENDENSE HUB APPLIANCE (SHA) UNIFIED DEPLOYMENT SCHEMA
-- =============================================================================
-- Database: migratekit_oma (kept for backward compatibility)
-- Created: 2025-10-05
-- Purpose: Complete SHA production deployment with backup capabilities
--
-- SCHEMA CONTENTS:
-- ----------------
-- 1. PRODUCTION OMA TABLES (35 tables)
--    - VM replication contexts and job tracking
--    - OSSEA volume management and NBD exports
--    - Failover jobs and network mappings
--    - VMA enrollment and SSH tunnel infrastructure
--    - Volume daemon operations and metrics
--    - Scheduler system (cron schedules, machine groups)
--    - JobLog tracking with correlation IDs
--
-- 2. SENDENSE BACKUP SYSTEM (6 new tables - Phase 1: VMware Backups)
--    - backup_repositories: Repository definitions (local/NFS/CIFS/S3/Azure)
--    - backup_policies: Retention policies and primary repository settings
--    - backup_copy_rules: Automated backup copy jobs configuration
--    - backup_jobs: Individual backup job tracking with progress
--    - backup_copies: Copy job status and verification tracking
--    - backup_chains: Backup chain management (full + incremental links)
--
-- TOTAL: 41 tables + 5 views
--
-- DEPLOYMENT NOTES:
-- - Database name: migratekit_oma (DO NOT CHANGE - hardcoded in binaries)
-- - Database user: oma_user / oma_password (standard credentials)
-- - All backup tables integrate with vm_replication_contexts (CASCADE DELETE)
-- - Backup system follows vm-centric architecture for consistency
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS `backup_repositories` (
  ...
  `immutable_config` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`immutable_config`)),
  `min_retention_days` int(11) DEFAULT 0,
  `total_size_bytes` bigint(20) DEFAULT 0,
  `used_size_bytes` bigint(20) DEFAULT 0,
  `available_size_bytes` bigint(20) DEFAULT 0,
  `last_check_at` timestamp NULL DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_name` (`name`),
  KEY `idx_type` (`repository_type`),
  KEY `idx_enabled` (`enabled`),
  KEY `idx_immutable` (`is_immutable`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `backup_policies` (
  ...
  UNIQUE KEY `unique_name` (`name`),
  KEY `idx_enabled` (`enabled`),
  KEY `fk_primary_repository` (`primary_repository_id`),
  CONSTRAINT `fk_primary_repository` FOREIGN KEY (`primary_repository_id`) REFERENCES `backup_repositories` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `backup_copy_rules` (
  ...
  `enabled` tinyint(1) DEFAULT 1,
  `verify_after_copy` tinyint(1) DEFAULT 1,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `idx_policy` (`policy_id`),
  KEY `idx_priority` (`priority`),
  KEY `fk_destination_repository` (`destination_repository_id`),
  CONSTRAINT `fk_copy_rule_policy` FOREIGN KEY (`policy_id`) REFERENCES `backup_policies` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_destination_repository` FOREIGN KEY (`destination_repository_id`) REFERENCES `backup_repositories` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `backup_jobs` (
  ...
  `parent_backup_id` varchar(64) DEFAULT NULL,
  `change_id` varchar(191) DEFAULT NULL,
  `bytes_transferred` bigint(20) DEFAULT 0,
  `total_bytes` bigint(20) DEFAULT 0,
  ...
  `completed_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_vm_context` (`vm_context_id`),
  KEY `idx_repository` (`repository_id`),
  KEY `idx_policy` (`policy_id`),
  KEY `idx_status` (`status`),
  KEY `idx_created` (`created_at`),
  KEY `idx_parent` (`parent_backup_id`),
  CONSTRAINT `fk_backup_vm_context` FOREIGN KEY (`vm_context_id`) REFERENCES `vm_replication_contexts` (`context_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_backup_repository` FOREIGN KEY (`repository_id`) REFERENCES `backup_repositories` (`id`),
  CONSTRAINT `fk_backup_policy` FOREIGN KEY (`policy_id`) REFERENCES `backup_policies` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_parent_backup` FOREIGN KEY (`parent_backup_id`) REFERENCES `backup_jobs` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `backup_copies` (
  ...
  `repository_id` varchar(64) NOT NULL,
  `copy_rule_id` varchar(64) DEFAULT NULL,
  `status` enum('pending','copying','verifying','completed','failed') DEFAULT 'pending',
  `file_path` varchar(512) NOT NULL,
  `size_bytes` bigint(20) DEFAULT 0,
  `copy_started_at` timestamp NULL DEFAULT NULL,
  `copy_completed_at` timestamp NULL DEFAULT NULL,
  ...
  KEY `idx_source_backup` (`source_backup_id`),
  KEY `idx_repository` (`repository_id`),
  KEY `idx_status` (`status`),
  KEY `fk_copy_rule` (`copy_rule_id`),
  CONSTRAINT `fk_source_backup` FOREIGN KEY (`source_backup_id`) REFERENCES `backup_jobs` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_copy_repository` FOREIGN KEY (`repository_id`) REFERENCES `backup_repositories` (`id`),
  CONSTRAINT `fk_copy_rule` FOREIGN KEY (`copy_rule_id`) REFERENCES `backup_copy_rules` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `backup_chains` (
  ...
  KEY `idx_vm_context` (`vm_context_id`),
  KEY `fk_full_backup` (`full_backup_id`),
  KEY `fk_latest_backup` (`latest_backup_id`),
  CONSTRAINT `fk_chain_vm_context` FOREIGN KEY (`vm_context_id`) REFERENCES `vm_replication_contexts` (`context_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_full_backup` FOREIGN KEY (`full_backup_id`) REFERENCES `backup_jobs` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_latest_backup` FOREIGN KEY (`latest_backup_id`) REFERENCES `backup_jobs` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

...
// GetOrCreateChainWithBackup retrieves an existing chain or creates a new one with the given backup.
// For full backups, it sets full_backup_id to the backup_id. For incrementals, it updates latest_backup_id.
func (cm *ChainManager) GetOrCreateChainWithBackup(ctx context.Context, vmContextID string, diskID int, backupID string, backupType BackupType) (*BackupChain, error) {
	// Try to get existing chain
	chain, err := cm.GetChain(ctx, vmContextID, diskID)
	if err == nil {
		// Chain exists - update latest_backup_id
		chain.LatestBackupID = backupID
		chain.UpdatedAt = time.Now()
		if backupType == BackupTypeFull {
			chain.FullBackupID = backupID // Update full backup if this is a new full backup
		}
		// Update chain in database
		updateErr := cm.repo.UpdateBackupChain(ctx, chain)
		if updateErr != nil {
			return nil, &ChainError{
				ChainID: chain.ID,
				Op:      "update",
				Err:     fmt.Errorf("failed to update chain: %w", updateErr),
			}
		}
		return chain, nil
	}
	...
	// Create new chain with backup references
	chainID := GenerateChainID(vmContextID, diskID)
	now := time.Now()

	// For new chains, both full and latest point to the first backup (which must be a full backup)
	newChain := &BackupChain{
		ID:             chainID,
		VMContextID:    vmContextID,
		DiskID:         diskID,
		FullBackupID:   backupID, // Set to actual backup_id to satisfy FK constraint
		LatestBackupID: backupID, // Set to actual backup_id to satisfy FK constraint
		Backups:        []*Backup{},
		TotalBackups:   0,
		TotalSizeBytes: 0,
		CreatedAt:      now,

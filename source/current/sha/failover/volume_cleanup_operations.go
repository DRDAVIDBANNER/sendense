...
// ReattachVolumesToOMA reattaches volumes to original SHA VM using Volume Daemon
func (vco *VolumeCleanupOperations) ReattachVolumesToOMA(ctx context.Context, volumeIDs []string, vmNameOrID string) error {
	logger := vco.jobTracker.Logger(ctx)
	logger.Info("🔗 Reattaching volumes to SHA via Volume Daemon", "volume_count", len(volumeIDs), "vm_name_or_id", vmNameOrID)

	if len(volumeIDs) == 0 {
		logger.Info("No volumes to reattach to SHA")
		return nil
	}

	// Get SHA VM ID from database (following Volume Daemon v2.1.2 pattern)
	shaVMID, err := vco.getOMAVMIDFromDatabase(ctx)
	if err != nil {
		return fmt.Errorf("failed to get SHA VM ID from database: %w", err)
	}
	logger.Info("Retrieved SHA VM ID from database for volume reattachment via Volume Daemon", "oma_vm_id", shaVMID)

	var attachOperations []string

	// Reattach each volume to SHA using Volume Daemon
	for _, volumeID := range volumeIDs {
		logger.Info("🔗 Reattaching volume to SHA via Volume Daemon", "volume_id", volumeID, "oma_vm_id", shaVMID)

		operation, err := vco.volumeClient.AttachVolume(context.Background(), volumeID, shaVMID)
		if err != nil {
			logger.Error("Failed to initiate volume reattachment to SHA via Volume Daemon", "error", err, "volume_id", volumeID, "oma_vm_id", shaVMID)
			return fmt.Errorf("failed to reattach volume %s to SHA: %w", volumeID, err)
		}

		attachOperations = append(attachOperations, operation.ID)
		logger.Info("Volume reattachment to SHA initiated via Volume Daemon", "volume_id", volumeID, "operation_id", operation.ID)
	}
	...
	for i, operationID := range attachOperations {
		...
		logger.Info("Volume reattached to SHA successfully",
			"volume_id", volumeID,
	...
	logger.Info("✅ All volumes reattached to SHA successfully via Volume Daemon",
		"oma_vm_id", shaVMID,
...
func (vco *VolumeCleanupOperations) getOMAVMIDFromDatabase(ctx context.Context) (string, error) {
	...
	var shaVMID string
	err := vco.db.GetGormDB().Raw("SELECT oma_vm_id FROM ossea_configs WHERE is_active = 1 LIMIT 1").Scan(&shaVMID).Error
	if err != nil {

...
// =============================================================================
// IMPROVED APPROACH: Uses SNA API validation + progress stagnation + impossible states
...
// SNAJobStatus represents the status of a job from SNA perspective
type SNAJobStatus string

const (
	SNAJobFound    SNAJobStatus = "found"     // SNA knows about the job
	SNAJobNotFound SNAJobStatus = "not_found" // SNA doesn't know about job = phantom
	SNAJobNoData   SNAJobStatus = "no_data"   // SNA unreachable or no data
	SNAJobError    SNAJobStatus = "error"     // API error occurred
)

// SNAJobResponse represents SNA API response for job status
type SNAJobResponse struct {
...
type PhantomDetectionResult struct {
	...
	SNAStatus        SNAJobStatus    `json:"vma_status"`
	...
	SNAData          *SNAJobResponse `json:"vma_data,omitempty"`
}
...
func (p *PhantomJobDetector) DetectPhantomJob(ctx context.Context, job *database.ReplicationJob) (*PhantomDetectionResult, error) {
	...
	// TIER 1: SNA API Validation (Most Reliable)
	snaStatus, snaData, err := p.checkVMAJobStatus(ctx, job.ID)
	result.SNAStatus = snaStatus
	result.SNAData = snaData

	if err != nil {
		logger.Error("SNA API check failed", "error", err, "job_id", job.ID)
		result.SNAStatus = SNAJobError
	}

	// If SNA doesn't know about the job, it's definitely phantom
	if snaStatus == SNAJobNotFound {
		result.IsPhantom = true
		result.Reason = "SNA API reports job not found - definitive phantom"
		logger.Warn("PHANTOM DETECTED: SNA doesn't know about job", "job_id", job.ID)
		return result, nil
	}

	// TIER 2: Progress Stagnation Detection
	progressStagnant := p.checkProgressStagnation(job, snaStatus)
	result.ProgressStagnant = progressStagnant

	if progressStagnant {
		result.IsPhantom = true
		result.Reason = "Progress stagnation: no updates >2 hours AND no SNA data"
		logger.Warn("PHANTOM DETECTED: Progress stagnation",
			"job_id", job.ID,
			"last_update", job.UpdatedAt,
			"vma_status", snaStatus,
		)
		return result, nil
	}
...
// checkVMAJobStatus queries SNA API to verify job existence
func (p *PhantomJobDetector) checkVMAJobStatus(ctx context.Context, jobID string) (SNAJobStatus, *SNAJobResponse, error) {
	if p.snaAPIEndpoint == "" {
		return SNAJobNoData, nil, fmt.Errorf("SNA API endpoint not configured")
	}

	// Build SNA API URL for job status
	url := fmt.Sprintf("%s/api/v1/jobs/%s/status", p.snaAPIEndpoint, jobID)

	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		return SNAJobError, nil, fmt.Errorf("failed to create SNA request: %w", err)
	}

	resp, err := p.httpClient.Do(req)
	if err != nil {
		return SNAJobNoData, nil, fmt.Errorf("SNA API request failed: %w", err)
	}
	...
	case http.StatusOK:
		var snaResp SNAJobResponse
		if err := json.NewDecoder(resp.Body).Decode(&snaResp); err != nil {
			return SNAJobError, nil, fmt.Errorf("failed to decode SNA response: %w", err)
		}
		return SNAJobFound, &snaResp, nil

	case http.StatusNotFound:
		// SNA doesn't know about this job = phantom
		return SNAJobNotFound, nil, nil

	default:
		return SNAJobError, nil, fmt.Errorf("SNA API returned status %d", resp.StatusCode)
	}
}

// checkProgressStagnation detects stagnant progress combined with no SNA data
func (p *PhantomJobDetector) checkProgressStagnation(job *database.ReplicationJob, snaStatus SNAJobStatus) bool {
	// Only consider stagnant if no updates >2 hours AND no SNA data
	return time.Since(job.UpdatedAt) > 2*time.Hour && snaStatus == SNAJobNoData
}

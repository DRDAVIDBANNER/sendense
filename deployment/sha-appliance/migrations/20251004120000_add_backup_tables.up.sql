-- Migration: Add Backup Repository Tables
-- Date: 2025-10-04
-- Purpose: Add tables for backup repository system (Phase 1 - VMware Backups)
...
-- Backup policies (3-2-1 rule, etc.)
...
-- Backup jobs (extends replication system with backup capability)
...
-- Backup copies (tracks copies to other repositories)
...
-- Backup chains (tracks full → incremental relationships)
CREATE TABLE IF NOT EXISTS backup_chains (
    id VARCHAR(64) PRIMARY KEY,
    vm_context_id VARCHAR(191) NOT NULL,
    disk_id INT NOT NULL,
    full_backup_id VARCHAR(64) NOT NULL DEFAULT '',
    latest_backup_id VARCHAR(64) NOT NULL DEFAULT '',

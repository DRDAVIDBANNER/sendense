...
func NewReplicationHandler(db database.Connection, mountManager *volume.MountManager, vmaProgressPoller workflows.VMAProgressPoller) *ReplicationHandler {
	return &ReplicationHandler{
		db:              db,
		migrationEngine: workflows.NewMigrationEngine(db, mountManager, vmaProgressPoller),
...
// GetReplicationProgress handles real-time progress requests with VMA integration
// @Summary Get real-time replication progress
// @Description Get detailed replication progress including VMA throughput and phase tracking
// @Tags replications
// @Security BearerAuth
// @Produce json
...
func (h *ReplicationHandler) GetReplicationProgress(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("job_id", jobID).Debug("Getting replication progress with VMA integration")
	...
	progressResp := ReplicationProgressResponse{
		...
		// VMA Progress Integration
		VMAProgress: VMAProgressInfo{
			SyncType:            job.VMASyncType,
			CurrentPhase:        job.VMACurrentPhase,
			ThroughputMBps:      job.VMAThroughputMBps,
			ETASeconds:          job.VMAETASeconds,
			LastPolled:          job.VMALastPollAt,
			ErrorClassification: job.VMAErrorClassification,
			ErrorDetails:        job.VMAErrorDetails,
		},
	}

	// Calculate ETA string
	if job.VMAETASeconds != nil && *job.VMAETASeconds > 0 {
		eta := time.Duration(*job.VMAETASeconds) * time.Second
		progressResp.VMAProgress.ETAFormatted = eta.String()
	}

	// Add real-time status
	progressResp.VMAProgress.IsLive = job.VMALastPollAt != nil &&
		time.Since(*job.VMALastPollAt) < 30*time.Second

	log.WithFields(log.Fields{
		"job_id":           jobID,
		"progress_percent": job.ProgressPercent,
		"vma_phase":        job.VMACurrentPhase,
		"vma_throughput":   job.VMAThroughputMBps,
		"vma_last_poll":    job.VMALastPollAt,
	}).Debug("✅ Successfully retrieved replication progress with VMA data")

	h.writeJSONResponse(w, http.StatusOK, progressResp)
}

// ReplicationProgressResponse represents enhanced progress response with VMA integration
type ReplicationProgressResponse struct {
	...
	// VMA Progress Integration
	VMAProgress VMAProgressInfo `json:"vma_progress"`
}

// VMAProgressInfo represents VMA-specific progress information
type VMAProgressInfo struct {
...
// GetVMAProgressProxy proxies VMA progress requests through the tunnel
// @Summary Get VMA progress via tunnel
// @Description Proxy VMA progress API through OMA tunnel following project rules (port 443 only)
// @Tags progress
// @Security BearerAuth
// @Produce json
// @Param job_id path string true "Job ID"
...
func (h *ReplicationHandler) GetVMAProgressProxy(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("job_id", jobID).Debug("Proxying VMA progress request through tunnel")

	// Create HTTP client for VMA API call (following project rules: all traffic via tunnel)
	vmaURL := fmt.Sprintf("http://localhost:9081/api/v1/progress/%s", jobID)

	client := &http.Client{
		Timeout: 10 * time.Second,
	}

	// Make request to VMA API
	resp, err := client.Get(vmaURL)
	if err != nil {
		log.WithError(err).WithField("job_id", jobID).Error("Failed to connect to VMA progress API")
		h.writeErrorResponse(w, http.StatusServiceUnavailable, "VMA API unavailable", err.Error())
		return
	}
	defer resp.Body.Close()

	// Handle VMA API response codes
	if resp.StatusCode == http.StatusNotFound {
		log.WithField("job_id", jobID).Debug("Job not found in VMA API")
		h.writeErrorResponse(w, http.StatusNotFound, "Job not found", "Migration job not found in VMA")
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		}).Warn("VMA API returned non-200 status")
		h.writeErrorResponse(w, resp.StatusCode, "VMA API error", fmt.Sprintf("VMA returned status %d", resp.StatusCode))
		return
	}

	// Read and forward VMA response
	...
	}).Debug("✅ Successfully proxied VMA progress response")
}

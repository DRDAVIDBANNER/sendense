...
// SHANBDRepository implements NBD export operations using the existing migratekit_oma database
type SHANBDRepository struct {
	db *sqlx.DB
}

// SHANBDExport represents the existing nbd_exports table structure in migratekit_oma
type SHANBDExport struct {
...
func NewOMANBDRepository(db *sqlx.DB) nbd.ExportRepository {
	return &SHANBDRepository{db: db}
}

// CreateExport creates a new NBD export record in the existing migratekit_oma database
func (r *SHANBDRepository) CreateExport(ctx context.Context, export *nbd.ExportInfo) error {
	// Metadata is stored in our export metadata field, not in the SHA table
	// The SHA table doesn't have a metadata column, so we'll store key info in our interface
...
func (r *SHANBDRepository) UpdateExport(ctx context.Context, export *nbd.ExportInfo) error {
	query := `
		UPDATE nbd_exports 
...
func (r *SHANBDRepository) DeleteExport(ctx context.Context, exportID string) error {
...
func (r *SHANBDRepository) GetExport(ctx context.Context, exportID string) (*nbd.ExportInfo, error) {
	query := `
	...
	var shaExport SHANBDExport
	err := r.db.QueryRowContext(ctx, query, exportID).Scan(
		&shaExport.JobID,
		&shaExport.VolumeID,
		&shaExport.ExportName,
		&shaExport.Port,
		&shaExport.DevicePath,
		&shaExport.ConfigPath,
		&shaExport.Status,
		&shaExport.CreatedAt,
		&shaExport.UpdatedAt,
	)
	...
	exportInfo := &nbd.ExportInfo{
		ID:         fmt.Sprintf("%d", shaExport.ID),
		VolumeID:   shaExport.VolumeID,
		ExportName: shaExport.ExportName,
		DevicePath: shaExport.DevicePath,
		Port:       shaExport.Port,
		Status:     nbd.ExportStatus(shaExport.Status),
		CreatedAt:  shaExport.CreatedAt,
		UpdatedAt:  shaExport.UpdatedAt,
		Metadata: map[string]string{
			"job_id":      stringPtrToString(shaExport.JobID),
			"config_path": shaExport.ConfigPath,
		},
	}
...
func (r *SHANBDRepository) GetExportByVolumeID(ctx context.Context, volumeID string) (*nbd.ExportInfo, error) {
	query := `
		SELECT job_id, volume_id, export_name, port, device_path, 
		       config_path, status, created_at, updated_at
		FROM nbd_exports 
...
func (r *SHANBDRepository) ListExports(ctx context.Context, filter nbd.ExportFilter) ([]*nbd.ExportInfo, error) {
	query := `
		SELECT job_id, volume_id, export_name, port, device_path, 
		       config_path, status, created_at, updated_at
		FROM nbd_exports 
...
func (r *SHANBDRepository) getVMContextIDForJob(jobID string) (string, error) {
	var vmContextID string
...
func (r *SHANBDRepository) getVMContextIDFromVolumeID(volumeID string) (string, error) {

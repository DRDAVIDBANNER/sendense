...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const response = await fetch(`${OMA_API_BASE}/api/v1/admin/vma/reject/${id}`, {
      method: 'POST',
      headers: {
    ...
    console.error('VMA enrollment rejection failed:', error);
    return NextResponse.json(
      { error: 'Failed to reject VMA enrollment' },
      { status: 500 }
    );
  }
}

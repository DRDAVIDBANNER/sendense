...
type VolumeDetails struct {
	...
	DevicePath string `json:"device_path"` // SHA device path (/dev/vdb, /dev/vdc)
...
func (ufe *UnifiedFailoverEngine) executeSourceVMPowerOffPhase(ctx context.Context, jobID string, config *UnifiedFailoverConfig) error {
	return ufe.jobTracker.RunStep(ctx, jobID, "source-vm-power-off", func(ctx context.Context) error {
		...
		if ufe.snaClient == nil {
			return fmt.Errorf("SNA client not available for power management")
		}
		...
		powerState, err := ufe.snaClient.GetVMPowerState(ctx, config.VMwareVMID, vcenterHost, vcenterUsername, vcenterPassword)
		if err != nil {
		...
		if err := ufe.snaClient.PowerOffSourceVM(ctx, config.VMwareVMID, vcenterHost, vcenterUsername, vcenterPassword); err != nil {
			return fmt.Errorf("failed to power off source VM: %w", err)
		}

		// Verify final power state
		finalState, err := ufe.snaClient.GetVMPowerState(ctx, config.VMwareVMID, vcenterHost, vcenterUsername, vcenterPassword)
		if err != nil {
...
func (ufe *UnifiedFailoverEngine) executeFinalSyncPhase(ctx context.Context, jobID string, config *UnifiedFailoverConfig) error {
	return ufe.jobTracker.RunStep(ctx, jobID, "final-sync", func(ctx context.Context) error {
		...
		// Step 2: Fresh VM discovery from SNA (following standard pattern)
		...
		// Step 4: Call standard SHA replication API
		...
		// Step 5: Wait for completion using existing SNA poller pattern
...
func (ufe *UnifiedFailoverEngine) executeVolumeAttachmentPhase(ctx context.Context, jobID string, config *UnifiedFailoverConfig, destinationVMID string) error {
	return ufe.jobTracker.RunStep(ctx, jobID, "volume-attachment", func(ctx context.Context) error {
		...
		// Step 2: Detach OS volume from SHA
		logger.Info("🔗 Detaching OS volume from SHA", "volume_id", multiDiskInfo.OSVolume.VolumeID)
		if err := ufe.volumeOperations.DetachVolumeFromOMA(ctx, multiDiskInfo.OSVolume.VolumeID); err != nil {
			return fmt.Errorf("failed to detach OS volume from SHA: %w", err)
		}
		...
		if err := ufe.attachVolumeToDestinationVMAsRoot(ctx, multiDiskInfo.OSVolume.VolumeID, destinationVMID); err != nil {
			// Critical error - try to reattach OS volume to SHA
			...
			if reattachErr := ufe.volumeOperations.ReattachVolumeToOMA(ctx, multiDiskInfo.OSVolume.VolumeID); reattachErr != nil {
				logger.Error("❌ Recovery failed: Could not reattach OS volume to SHA", "error", reattachErr)
			}
		...
		for i, dataVolume := range multiDiskInfo.DataVolumes {
			...
			// Detach data volume from SHA
			logger.Info("🔗 Detaching data volume from SHA", "volume_id", dataVolume.VolumeID)
			if err := ufe.volumeOperations.DetachVolumeFromOMA(ctx, dataVolume.VolumeID); err != nil {
				logger.Error("Failed to detach data volume from SHA", "error", err, "disk_id", dataVolume.DiskID)
				return fmt.Errorf("failed to detach data volume %s from SHA: %w", dataVolume.DiskID, err)
			}
			...
			if err := ufe.attachVolumeToDestinationVM(ctx, dataVolume.VolumeID, destinationVMID); err != nil {
				...
				// Try to reattach to SHA for recovery
				if reattachErr := ufe.volumeOperations.ReattachVolumeToOMA(ctx, dataVolume.VolumeID); reattachErr != nil {
					logger.Error("❌ Recovery failed: Could not reattach data volume to SHA", "error", reattachErr)
				}
...
// discoverVMFromVMA performs fresh VM discovery via SNA API using HTTP call
func (ufe *UnifiedFailoverEngine) discoverVMFromVMA(ctx context.Context, vmName, vCenterHost, datacenter string) (map[string]interface{}, error) {
	...
	encryptionService, err := services.NewCredentialEncryptionService()
	if err != nil {
		logger.Error("❌ Failed to initialize VMware credential encryption service for SNA discovery", "error", err.Error())
	...
	creds, err := credentialService.GetDefaultCredentials(ctx)
	if err != nil {
		logger.Error("❌ Failed to retrieve VMware credentials from database for SNA discovery", "error", err.Error())
	...
	logger.Info("✅ Using fresh VMware credentials from database for SNA discovery",
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal SNA discovery request: %w", err)
	}

	// Make HTTP POST call to SNA discovery API (same pattern as GUI)
	client := &http.Client{Timeout: 60 * time.Second}
	resp, err := client.Post("http://localhost:9081/api/v1/discover", "application/json", bytes.NewBuffer(requestBytes))
	if err != nil {
		return nil, fmt.Errorf("SNA discovery HTTP call failed: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		bodyBytes, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("SNA discovery API returned status %d: %s", resp.StatusCode, string(bodyBytes))
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&discoveryResponse); err != nil {
		return nil, fmt.Errorf("failed to decode SNA discovery response: %w", err)
	}
...
// callOMAReplicationAPI makes HTTP call to local SHA replication API
func (ufe *UnifiedFailoverEngine) callOMAReplicationAPI(ctx context.Context, request map[string]interface{}) (string, error) {
	...
	// Make HTTP POST to local SHA API (following documented pattern)
...
func (ufe *UnifiedFailoverEngine) waitForReplicationCompletion(ctx context.Context, jobID string) error {
	...
	// Start SNA polling for this job (SNA poller updates database automatically)
	logger.Info("🚀 Starting SNA progress polling for final sync", "job_id", jobID)
	...
	for {
		...
		case <-ticker.C:
			// Check job status in database (updated by SNA poller)

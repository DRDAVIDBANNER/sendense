...
func (v *CloudStackPrerequisiteValidator) ValidateAll(ctx context.Context, config *CloudStackConfig) (*ValidationReport, error) {
	...
	// Category 7: SHA VM Configuration
...
func (v *CloudStackPrerequisiteValidator) validateOMAVM(ctx context.Context, config *CloudStackConfig, report *ValidationReport) {
	if config.SHAVMID == "" {
		report.Results = append(report.Results, ValidationResult{
			Category:    "SHA VM Configuration",
			CheckName:   "SHA VM ID Specified",
			Passed:      false,
			Message:     "No SHA VM ID specified in configuration",
			Details:     "SHA VM ID is required for volume attachment operations",
			Fix:         "Specify the CloudStack VM ID of this SHA appliance",
			AutoFixable: true,
			Severity:    "critical",
		})
		return
	...
	// Check 7.1: SHA VM Exists and is accessible
	vm, err := v.client.GetVMByID(config.SHAVMID)
	if err != nil {
		report.Results = append(report.Results, ValidationResult{
			Category:    "SHA VM Configuration",
			CheckName:   "SHA VM Exists",
			Passed:      false,
			Message:     fmt.Sprintf("Cannot find SHA VM with ID '%s'", config.SHAVMID),
			Details:     fmt.Sprintf("Error: %v", err),
			Fix:         "Verify the SHA VM ID is correct, check VM exists in CloudStack",
			AutoFixable: false,
			Severity:    "critical",
		})
		return
	...
	report.Results = append(report.Results, ValidationResult{
		...
		CheckName:   "SHA VM Exists",
		Passed:      true,
		Message:     fmt.Sprintf("SHA VM found: %s (State: %s)", vm.DisplayName, vm.State),
		AutoFixable: false,
		Severity:    "info",
	})
	
	// Check 7.2: SHA VM is running
	if vm.State != "Running" {
		report.Results = append(report.Results, ValidationResult{
			Category:    "SHA VM Configuration",
			CheckName:   "SHA VM Running",
			Passed:      false,
			Message:     fmt.Sprintf("SHA VM is not running (State: %s)", vm.State),
			Details:     "SHA VM must be running for volume operations",
			Fix:         "Start the SHA VM before proceeding with migrations",
			AutoFixable: false,
			Severity:    "critical",
		})
		return
...
func (v *CloudStackPrerequisiteValidator) validateVolumePrerequisites(ctx context.Context, config *CloudStackConfig, report *ValidationReport) {
	...
	// This is validated implicitly through SHA VM existence check
	report.Results = append(report.Results, ValidationResult{
		Category:    "Volume Operations",

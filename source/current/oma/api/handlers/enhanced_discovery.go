...
type DiscoveredVMInfo struct {
	...
	Disks      []services.VMADiskInfo     `json:"disks"`
	Networks   []services.VMANetworkInfo  `json:"networks"`
...
// BulkAddVMsRequest represents a request to bulk add VMs to OMA
...
// AddVMsRequest represents a request to add specific VMs to OMA without jobs
...
// AddVMsResponse represents the response from adding VMs to OMA
...
// DiscoverVMs discovers VMs from VMA with optional context creation
...
func (h *EnhancedDiscoveryHandler) DiscoverVMs(w http.ResponseWriter, r *http.Request) {
	...
	// Discover VMs from VMA (using resolved credentials)
	...
	if err != nil {
		log.WithError(err).Error("Failed to discover VMs from VMA")
...
// BulkAddVMs adds multiple VMs to OMA without creating replication jobs
...
func (h *EnhancedDiscoveryHandler) BulkAddVMs(w http.ResponseWriter, r *http.Request) {
	...
	}).Info("Starting bulk VM addition to OMA")
	...
	if err != nil {
		log.WithError(err).Error("Failed to bulk add VMs to OMA")
...
// AddVMs adds specific VMs to OMA without creating replication jobs (simplified interface)
...
func (h *EnhancedDiscoveryHandler) AddVMs(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("vm_count", len(request.VMNames)).Info("Adding specific VMs to OMA")
	...
	}).Info("VMs added to OMA successfully")
...
func (h *EnhancedDiscoveryHandler) GetDiscoveryPreview(w http.ResponseWriter, r *http.Request) {
	...
	// Discover VMs from VMA (preview only)
	vmaResponse, err := h.discoveryService.DiscoverVMsFromVMA(ctx, request)
	if err != nil {
	...
	previewVMs := make([]DiscoveredVMInfo, 0, len(vmaResponse.VMs))
	for _, vm := range vmaResponse.VMs {
		existing, _ := h.vmContextRepo.GetVMContextByName(vm.Name)

...
// DetachVolumeFromOMA detaches volume from SHA using Volume Daemon
func (vo *VolumeOperations) DetachVolumeFromOMA(ctx context.Context, volumeID string) error {
	logger := vo.jobTracker.Logger(ctx)
	logger.Info("🔌 Detaching volume from SHA via Volume Daemon", "volume_id", volumeID)
	...
	logger.Info("✅ Volume detached from SHA successfully",
		"volume_id", volumeID,
...
func (vo *VolumeOperations) ReattachVolumeToOMA(ctx context.Context, volumeID string) error {
	logger := vo.jobTracker.Logger(ctx)
	logger.Warn("🚨 Emergency: Reattaching volume to SHA via Volume Daemon", "volume_id", volumeID)

	// Get SHA VM ID from configuration
	shaVMID, err := vo.getOMAVMID(ctx, volumeID)
	if err != nil {
		return fmt.Errorf("failed to get SHA VM ID: %w", err)
	}

	operation, err := vo.volumeClient.AttachVolume(context.Background(), volumeID, shaVMID)
	if err != nil {
		return fmt.Errorf("failed to reattach volume to SHA: %w", err)
	}
...
func (vo *VolumeOperations) getOMAVMID(ctx context.Context, volumeID string) (string, error) {
	// Use helper method to get SHA VM ID from configuration

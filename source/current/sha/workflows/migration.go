...
// SNAProgressPoller interface for SNA progress polling service
type SNAProgressPoller interface {
...
type MigrationEngine struct {
	...
	snaProgressPoller SNAProgressPoller // Interface for SNA progress polling
}

// NewMigrationEngine creates a new migration workflow engine
func NewMigrationEngine(db database.Connection, mountManager *volume.MountManager, snaProgressPoller SNAProgressPoller) *MigrationEngine {
...
func (m *MigrationEngine) StartMigration(ctx context.Context, req *MigrationRequest) (*MigrationResult, error) {
	...
	// Phase 4: Attach volumes to SHA appliance (but don't mount as filesystems)
	// NBD needs raw block device access, so we attach but don't mount
	log.Info("Attaching OSSEA volumes to SHA appliance for NBD access")
	...
	// Phase 8: Initiate VMware replication via SNA API
	log.WithField("job_id", req.JobID).Info("Initiating VMware replication via SNA")
	...
	// Update status to replicating with NBD export name and start SNA progress polling
	...
	// Start SNA progress polling for real-time updates
	if m.snaProgressPoller != nil {
		if err := m.snaProgressPoller.StartPolling(req.JobID); err != nil {
			log.WithError(err).WithField("job_id", req.JobID).Warn("Failed to start SNA progress polling - continuing with static progress")
		} else {
			log.WithField("job_id", req.JobID).Info("🚀 Started SNA progress polling - real-time progress tracking active")
		}
	}

	result.Status = "replicating"
	result.ProgressPercent = 0.0 // SNA will update with real progress during replication
...
func (m *MigrationEngine) analyzeAndRecordVMDisks(req *MigrationRequest) error {
	...
	for i, disk := range req.SourceVM.Disks {
		vmDisk := &database.VMDisk{
			JobID:            &req.JobID, // Pointer for nullable field
		...
		if existingDisk != nil {
			...
			existingDisk.JobID = &req.JobID // Pointer for nullable field
...
// updateSetupProgress updates the setup progress and status for SHA pre-replication phases
...
// updateJobStatusWithNBDExport updates job status and populates nbd_export_name for SNA progress polling
func (m *MigrationEngine) updateJobStatusWithNBDExport(jobID, status string, progressPercent float64, nbdExports []*nbd.ExportInfo) error {
	...
	// Populate nbd_export_name from the first NBD export for SNA progress polling
	...
	if len(nbdExports) > 0 {
		...
		}).Info("✅ Populated nbd_export_name for SNA progress polling")
	} else {
...
// initiateVMwareReplication calls SNA API to start actual replication
func (m *MigrationEngine) initiateVMwareReplication(req *MigrationRequest, nbdExports []*nbd.ExportInfo) error {
	log.WithField("job_id", req.JobID).Info("Calling SNA API to initiate VMware replication")
	...
	if err != nil {
		...
		if err != nil {
			...
			log.WithField("vcenter_host", creds.VCenterHost).Info("✅ Using credential service for SNA replication")
			vcenterUsername = creds.Username
			vcenterPassword = creds.Password
		}
	}

	// Build NBD target information for SNA (NBD connection details)
	var nbd_targets []map[string]interface{}
	// Resolve SHA NBD host from environment to avoid hardcoding
	shaNbdHost := os.Getenv("SHA_NBD_HOST")
	if shaNbdHost == "" {
		// If not set, prefer passing only port+export and let SNA resolve host via its own env
		log.Warn("SHA_NBD_HOST not set on SHA; sending NBD targets without host, SNA must compose using its environment")
	}
	...
	for i, export := range nbdExports {
		...
		if shaNbdHost != "" {
			...
			devicePath = fmt.Sprintf("nbd://%s:%d/%s", shaNbdHost, export.Port, export.ExportName)
		} else {
			// Hostless form hint; SNA will substitute host using its SHA_NBD_HOST
	...
	// Debug: Log NBD targets being sent to SNA
	log.WithFields(log.Fields{
		"job_id":            req.JobID,
		"nbd_exports_count": len(nbdExports),
		"nbd_targets_count": len(nbd_targets),
		"nbd_targets":       nbd_targets,
	}).Info("Built NBD targets for SNA API call")

	// SNA API request payload with NBD target information
	// Get SHA URL from environment, fallback to localhost for tunnel
	shaURL := os.Getenv("SHA_API_URL")
	if shaURL == "" {
		shaURL = "http://localhost:8082" // Default for reverse tunnel
	}
	...
	// Convert to JSON
	jsonData, err := json.Marshal(snaRequest)
	if err != nil {
		return fmt.Errorf("failed to marshal SNA request: %w", err)
	}

	// Get SNA API URL from environment, fallback to localhost for tunnel
	snaAPIURL := os.Getenv("SNA_API_URL")
	if snaAPIURL == "" {
		snaAPIURL = "http://localhost:9081" // Default for reverse tunnel
	}

	// Make HTTP request to SNA via reverse tunnel
	client := &http.Client{Timeout: 30 * time.Second}
	resp, err := client.Post(snaAPIURL+"/api/v1/replicate", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("SNA API returned status %d", resp.StatusCode)
	}
	...
	}).Info("✅ VMware replication initiated via SNA API")
...
// attachOSSEAVolumes attaches volumes to SHA appliance for NBD access (without mounting)
func (m *MigrationEngine) attachOSSEAVolumes(ctx context.Context, req *MigrationRequest, volumeResults []VolumeProvisionResult) ([]VolumeMountResult, error) {
	log.WithField("job_id", req.JobID).Info("Attaching OSSEA volumes to SHA appliance")
	...
	// Validate SHA VM ID is configured
	if osseaConfig.SHAVMID == "" {
		return nil, fmt.Errorf("SHA VM ID not configured in OSSEA configuration")
	}
	...
	for _, volumeResult := range volumeResults {
		...
		// Attach volume to SHA appliance using Volume Management Daemon
		log.WithFields(log.Fields{
			"job_id":    req.JobID,
			"volume_id": volumeResult.OSSEAVolumeID,
			"oma_vm_id": osseaConfig.SHAVMID,
		}).Info("Attaching OSSEA volume to SHA appliance via Volume Daemon")

		// Use Volume Management Daemon for attachment with real device correlation
		volumeClient := common.NewVolumeClient("http://localhost:8090")

		operation, err := volumeClient.AttachVolume(ctx, volumeResult.OSSEAVolumeID, osseaConfig.SHAVMID)
		if err != nil {
		...
		log.WithFields(log.Fields{
			...
			"volume_id":   volumeResult.OSSEAVolumeID,
			"device_path": devicePath,
		}).Info("✅ OSSEA volume attached to SHA appliance")
	}

	log.WithFields(log.Fields{
		"job_id":        req.JobID,
		"volumes_count": len(attachResults),
	}).Info("✅ All OSSEA volumes attached to SHA appliance")

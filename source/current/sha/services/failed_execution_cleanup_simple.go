...
func (fecs *SimpleFailedExecutionCleanupService) CleanupFailedExecution(ctx context.Context, vmName string) error {
	...
	// Phase 1: Detach volumes from SHA
	...
	// Phase 3: Reattach volumes to SHA
	err = fecs.jobTracker.RunStep(ctx, jobID, "reattach-volumes", func(ctx context.Context) error {
		...
		for _, volume := range volumes {
			logger.Info("🔗 Reattaching volume to SHA", "volume_id", volume.VolumeID)
			_, attachErr := fecs.volumeClient.AttachVolume(ctx, volume.VolumeID, shaVMID)
			if attachErr != nil {

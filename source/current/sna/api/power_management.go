// Package api provides VMware power management endpoints for the SNA server
...
func (s *SNAControlServer) handleVMPowerOff(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	vmID := vars["vm_id"]
...
func (s *SNAControlServer) handleVMPowerOn(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	vmID := vars["vm_id"]
...
func (s *SNAControlServer) handleVMPowerState(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	vmID := vars["vm_id"]

	// Get credentials from query parameters (following existing SNA API pattern)

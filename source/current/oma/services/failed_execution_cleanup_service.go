...
func (fecs *FailedExecutionCleanupService) CleanupFailedExecution(ctx context.Context, vmName string) error {
	...
	// Phase 4: Reattach ALL volumes to OMA (both originally attached and detached)
	err = fecs.jobTracker.RunStep(ctx, jobID, "reattach-all-volumes", func(ctx context.Context) error {
		...
		for _, volume := range volumes {
			logger.Info("🔗 Reattaching volume to OMA", "volume_id", volume.VolumeID, "oma_vm_id", omaVMID)

			// Send attach request and get operation ID
			operation, attachErr := fecs.volumeClient.AttachVolume(ctx, volume.VolumeID, omaVMID)
			if attachErr != nil {

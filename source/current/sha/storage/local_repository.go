...
func (lr *LocalRepository) CreateBackup(ctx context.Context, req BackupRequest) (*Backup, error) {
	...
	// 🆕 FIX: Chain creation MOVED to after backup_job insert (was causing FK constraint violation)
	// The chain needs the backup_job record to exist first for the full_backup_id FK reference
	...
	// 🆕 FIX: Create or update backup chain AFTER backup_job record exists
	// This ensures the FK constraint is satisfied (full_backup_id references backup_jobs.id)
	//  For full backups, we set the full_backup_id to this backup's ID
	_, chainErr := lr.chainMgr.GetOrCreateChainWithBackup(ctx, req.VMContextID, req.DiskID, backupID, req.BackupType)
	if chainErr != nil {
		log.WithError(chainErr).WithFields(log.Fields{
			"backup_id":    backupID,
			"vm_context_id": req.VMContextID,
			"disk_id":      req.DiskID,
		}).Warn("Failed to create/update backup chain - backup created but chain tracking may be incomplete")
		// Don't fail the backup creation - chain can be created later
	}

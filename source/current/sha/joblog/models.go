...
type JobStart struct {
	...
	// ContextID links this job to a VM context for correlation
	ContextID *string `json:"context_id,omitempty"`

	// ExternalJobID stores external system job IDs (e.g., GUI-constructed IDs)
	ExternalJobID *string `json:"external_job_id,omitempty"`

	// JobCategory provides high-level categorization for filtering
...
type JobRecord struct {
	...
	ContextID   *string `db:"context_id"`   // VM context correlation
	JobCategory *string `db:"job_category"` // High-level categorization
}
...
type LogRecord struct {
	...
	Ts            time.Time `db:"ts"`
	ExternalJobID *string   `db:"external_job_id"` // GUI job ID correlation
}
...
const (
	...
	// externalJobIDKey is the context key for external job IDs (GUI correlation)
	externalJobIDKey contextKey = "joblog_external_job_id"
)
...
// WithExternalJobID adds an external job ID to the context
func WithExternalJobID(ctx context.Context, externalJobID string) context.Context {
	return context.WithValue(ctx, externalJobIDKey, externalJobID)
}

// ExternalJobIDFromCtx extracts the external job ID from the context
func ExternalJobIDFromCtx(ctx context.Context) (string, bool) {
	externalJobID, ok := ctx.Value(externalJobIDKey).(string)
	return externalJobID, ok
}

...
// ALIGNED: Uses same workflow as GUI with fresh VMA discovery + OMA API calls
type SchedulerService struct {
	...
	// ✅ NEW: VMA Discovery Integration (aligned with GUI workflow)
	vmaAPIEndpoint string
	vmaClient      *http.Client
	omaAPIEndpoint string
	omaClient      *http.Client
...
// ✅ VMA Discovery Integration (aligned with GUI workflow)
// These structures match the VMA API response format used by GUI

// VMDiscoveryRequest matches VMA API discovery request format
...
// VMDiscoveryResponse matches VMA API discovery response format
type VMDiscoveryResponse struct {
	VMs []VMDiscoveryData `json:"vms"`
}

// VMDiscoveryData matches the VM data structure from VMA discovery
...
// CreateMigrationRequest matches OMA API replication request format
...
// MigrationResult matches actual OMA API replication response format
...
// ✅ UPDATED: Now includes VMA discovery and OMA API integration (aligned with GUI workflow)
func NewSchedulerService(
	...
	phantomDetector := NewPhantomJobDetector(replicationRepo, jobTracker, vmaAPIEndpoint)
	...
	return &SchedulerService{
		...
		// ✅ NEW: VMA Discovery Integration (same as GUI)
		vmaAPIEndpoint: vmaAPIEndpoint,
		vmaClient:      &http.Client{Timeout: 30 * time.Second},
		omaAPIEndpoint: "http://localhost:8082", // Same endpoint as GUI
		omaClient:      &http.Client{Timeout: 60 * time.Second},
...
// 1. Fresh VMA discovery API call for latest VM specifications
// 2. OMA replication API call using same endpoint as GUI
...
func (s *SchedulerService) createReplicationJob(
	...
	// ✅ STEP 2: Transform to OMA API format (EXACT field mapping as GUI)
	...
	omaRequest := CreateMigrationRequest{
		...
		PreviousChangeID: "",
		SnapshotID:       "",
		// ✅ NEW: Scheduler metadata (passed to Migration Engine via OMA API)
		ScheduleExecutionID: execution.ID,
		VMGroupID:           group.ID,
		ScheduledBy:         "scheduler-service",
	}

	// ✅ STEP 3: Call OMA API (SAME endpoint and workflow as GUI)
	// This ensures identical behavior between manual and scheduled jobs
	result, err := s.callOMAReplicationAPI(ctx, omaRequest)
	if err != nil {
		logger.Error("OMA replication API call failed", "error", err)
		return "", fmt.Errorf("failed to start replication via OMA API: %w", err)
	}

	logger.Info("Successfully started replication via OMA API (aligned with GUI)",
		"job_id", result.JobID,
	...
	// No more direct database updates - metadata passed via OMA API request
	logger.Info("Scheduler metadata passed to Migration Engine via OMA API",
		"job_id", result.JobID,
...
// - getVMDisksForContext() - REPLACED by fresh VMA discovery API calls
//
...
// discoverVMFromVMA calls VMA discovery API to get fresh VM specifications
...
func (s *SchedulerService) discoverVMFromVMA(ctx context.Context, vmName, vCenterHost, datacenter string) (*VMDiscoveryData, error) {
	...
	// Call VMA discovery API (same endpoint as GUI)
	httpReq, err := http.NewRequestWithContext(ctx, "POST", s.vmaAPIEndpoint+"/api/v1/discover", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	logger.Info("Calling VMA discovery API for fresh VM data",
		"vm_name", vmName,
		"vcenter_host", vCenterHost,
		"datacenter", datacenter,
		"endpoint", s.vmaAPIEndpoint+"/api/v1/discover")

	resp, err := s.vmaClient.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("failed to call VMA discovery API: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("VMA discovery API returned status %d", resp.StatusCode)
	}
...
// callOMAReplicationAPI calls OMA replication API using the same endpoint as GUI
...
func (s *SchedulerService) callOMAReplicationAPI(ctx context.Context, req CreateMigrationRequest) (*MigrationResult, error) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, "POST", s.omaAPIEndpoint+"/api/v1/replications", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	logger.Info("Calling OMA replication API",
		"vm_name", req.SourceVM.Name,
		"replication_type", req.ReplicationType,
		"endpoint", s.omaAPIEndpoint+"/api/v1/replications")

	resp, err := s.omaClient.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("failed to call OMA replication API: %w", err)
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		return nil, fmt.Errorf("OMA replication API returned status %d", resp.StatusCode)
	}

	logger.Info("Successfully started replication via OMA API",
		"job_id", result.JobID,
		"status", result.Status,
		"progress_percent", result.ProgressPercent,
		"disks_count", len(result.Disks))

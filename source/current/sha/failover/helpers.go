...
import (
	...
	"github.com/vexxhost/migratekit-sha/common/logging"
...
// GetOMAVMID gets the SHA VM ID from system configuration
func (fh *FailoverHelpers) GetOMAVMID(ctx context.Context) (string, error) {
	logger := fh.jobTracker.Logger(ctx)

	// Get SHA VM ID from OSSEA configuration
	config, err := fh.GetOSSEAConfig()
	if err != nil {
		logger.Error("Failed to get OSSEA config for SHA VM ID", "error", err)
		return "", fmt.Errorf("failed to get OSSEA configuration: %w", err)
	}

	if config.SHAVMID == "" {
		logger.Error("SHA VM ID not configured in OSSEA config")
		return "", fmt.Errorf("SHA VM ID not configured in OSSEA config")
	}

	logger.Info("Retrieved SHA VM ID from configuration", "oma_vm_id", config.SHAVMID)
	return config.SHAVMID, nil
}

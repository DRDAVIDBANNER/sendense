...
// mountVolumesToOMA attaches and mounts volumes to the OMA appliance
func (m *MigrationEngine) mountVolumesToOMA(ctx context.Context, req *MigrationRequest) ([]VolumeMountResult, error) {
	log.WithField("job_id", req.JobID).Info("Mounting volumes to OMA appliance")
	...
	for _, vmDisk := range vmDisks {
		...
		// Step 1: Attach volume to OMA VM via Volume Daemon
		volumeClient := common.NewVolumeClient("http://localhost:8090")
		operation, err := volumeClient.AttachVolume(ctx, osseaVolume.VolumeID, osseaConfig.OMAVMID)
		if err != nil {
			result.Status = "attach_failed"
			result.ErrorMessage = err.Error()
			results = append(results, result)
			return results, fmt.Errorf("failed to attach volume %s to OMA: %w", osseaVolume.VolumeID, err)
		}
		...
		}).Info("Volume attached to OMA VM")
	...
	if osseaConfigID > 0 {
		...
		for _, vmDisk := range vmDisks {
			if vmDisk.OSSEAVolumeID > 0 {
				...
				// Detach volume from OMA VM via Volume Daemon
				volumeClient := common.NewVolumeClient("http://localhost:8090")

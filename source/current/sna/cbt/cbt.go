...
// NewManager creates a new CBT manager instance (backward compatibility)
...
// NewManagerWithDatacenter creates a new CBT manager instance with datacenter context
func NewManagerWithDatacenter(vcenter, username, password, datacenter string) *Manager {
	return &Manager{
		vcenter:    vcenter,
		username:   username,
		password:   password,
...
func (m *Manager) getVMAndProperties(ctx context.Context, vmPath string) (*object.VirtualMachine, mo.VirtualMachine, error) {
	...
	// Find the VM with datacenter context if available
	finder := find.NewFinder(client.Client, true)

	// Set datacenter context if available (improves VM discovery reliability)
	if m.datacenter != "" {
		dc, err := finder.Datacenter(ctx, m.datacenter)
		if err != nil {
			log.WithField("datacenter", m.datacenter).Warn("Could not find datacenter, proceeding without context")
		} else {
			finder.SetDatacenter(dc)
			log.WithField("datacenter", m.datacenter).Debug("Set datacenter context for VM discovery")
		}
	}

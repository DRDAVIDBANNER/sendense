 ...
 *         case of an error
 **/
// 🎯 MIGRATEKIT ENHANCEMENT: Cache flush functions for SIGHUP
static void refresh_existing_server_configs(GArray *servers, struct generic_conf *genconf) {
    msg(LOG_INFO, "🔄 MIGRATEKIT: Refreshing existing server configurations");
    ...
    if (!updated_servers) {
        msg(LOG_ERR, "🔄 MIGRATEKIT: Failed to re-parse config for refresh: %s", 
    ...
    for (int i = 0; i < servers->len; i++) {
        ...
        for (int j = 0; j < updated_servers->len; j++) {
            ...
            if (existing_server->servename && updated_server->servename &&
                strcmp(existing_server->servename, updated_server->servename) == 0) {
                
                // Update device path if changed
                if (strcmp(existing_server->exportname, updated_server->exportname) != 0) {
                    msg(LOG_INFO, "🔄 MIGRATEKIT: Updating device path for export %s: %s → %s",
                        existing_server->servename, 
    ...
    msg(LOG_INFO, "✅ MIGRATEKIT: Server configuration refresh completed");
}

static void clear_export_client_cache(void) {
    msg(LOG_INFO, "🧹 MIGRATEKIT: Clearing export client cache");
    ...
    msg(LOG_INFO, "✅ MIGRATEKIT: Export client cache cleared");
}
		...
		if (is_sigchld_caught) {
			...
			while ((pid=waitpid(-1, &status, WNOHANG)) > 0) {
                ...
                if (is_sighup_caught) {
                        ...
                        // 🎯 MIGRATEKIT ENHANCEMENT: Refresh existing server configurations
                        refresh_existing_server_configs(servers, genconf);

                        // 🎯 MIGRATEKIT ENHANCEMENT: Clear export client cache

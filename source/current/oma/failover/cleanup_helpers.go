...
func (ch *CleanupHelpers) GetOMAVMID(ctx context.Context, vmNameOrID string) (string, error) {
	logger := ch.jobTracker.Logger(ctx)
	logger.Info("🔍 Retrieving OMA VM ID from database for volume reattachment", "vm_name_or_id", vmNameOrID)

	var omaVMID string
	err := ch.db.GetGormDB().Raw("SELECT oma_vm_id FROM ossea_configs WHERE is_active = 1 LIMIT 1").Scan(&omaVMID).Error
	if err != nil {
	...
	logger.Info("Successfully retrieved OMA VM ID from database", "oma_vm_id", omaVMID)
	return omaVMID, nil
}

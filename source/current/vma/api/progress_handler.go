...
// VMAProgressResponse represents the response format expected by OMA Progress Poller
...
// convertToVMAProgressResponse converts internal ReplicationProgress to VMAProgressResponse format
func convertToVMAProgressResponse(jobProgress *progress.ReplicationProgress) *VMAProgressResponse {
	response := &VMAProgressResponse{
...
// GetJobProgress handles GET /progress/{jobId} requests for OMA Progress Poller
func (h *ProgressHandler) GetJobProgress(w http.ResponseWriter, r *http.Request) {
	...
	log.WithField("job_id", jobID).Info("🔍 VMA API: GET progress request received")
	log.WithField("job_id", jobID).Debug("VMA API: Getting progress for OMA poller")
	...
	if err != nil {
		if err.Error() == "job not found: "+jobID {
			log.WithField("job_id", jobID).Debug("VMA API: Job not found for progress request")
			http.Error(w, "job not found", http.StatusNotFound)
			return
		}
		log.WithError(err).WithField("job_id", jobID).Error("VMA API: Error getting job progress")
		http.Error(w, "internal server error", http.StatusInternalServerError)
		return
	}

	// Convert to VMAProgressResponse format for OMA
	vmaResponse := convertToVMAProgressResponse(jobProgress)
	...
	if err := json.NewEncoder(w).Encode(vmaResponse); err != nil {
		log.WithError(err).WithField("job_id", jobID).Error("VMA API: Failed to encode progress response")
	...
	log.WithFields(log.Fields{
		"job_id":     jobID,
		"percentage": vmaResponse.Percentage,
		"phase":      vmaResponse.Phase,
		"status":     vmaResponse.Status,
	}).Debug("VMA API: Successfully returned progress to OMA poller")
}
...
func (h *ProgressHandler) RegisterRoutes(router *mux.Router) {
	...
	// GET /api/v1/progress/{jobId} - Get progress for OMA poller
